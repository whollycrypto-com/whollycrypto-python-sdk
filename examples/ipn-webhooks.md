...
Merchant 5.0.1 adds `paid_asset_amount` (the full locked quote, including spread and rounding) and `paid_asset_amount_received` (the winning method's total valid receipts at settlement, including any funds awaiting confirmation). Both are exact decimal strings in `paid_asset` units. For 100 USDC requested and 99 accepted with tolerance, they are `"100"` and `"99"`; an overpayment of 101 is `"100"` and `"101"`. Never subtract tolerance from the requested amount or convert these strings to floating point for accounting.

The summary fields are null before settlement, after invalidation, for older settlements without a saved snapshot, or for manual acceptance without qualifying policy-final funds. The two new amount fields are also null in pre-5.0.1 snapshots, even if the paid asset is known. Missing prices alone leave the proven `paid_*` fields available and the rate null. A stale rate is explicitly flagged. Later payments using the same method, retries and redelivery never refresh the settlement snapshot, including a saved null. To see receipts at a later event, use `payment_info.methods[].amounts`; for current records, use the payments API. A genuine re-settlement or changed settling method captures a new snapshot: `observed_at` identifies that capture, while `settled_at` can retain the first settlement time. Old event bodies remain unchanged; payload version stays 2. Always check the signed invoice status and sequence; the summary is not permission to fulfil independently.
